...
export default function EditEvent({ params }: EditEventProps) {
    ...
    const { id } = use(params); // Desempaquetamos el ID de los parámetros de la URL de forma asíncrona
    ...
    // ==========================================
    // CUBRIMIENTO DE DATA AL MONTAR (CATÁLOGO + EVENTO)
    // ==========================================
    useEffect(() => {
        ...
        Promise.all([loadCatalog, loadEvent])
            .then(([catalogData, eventData]) => {
                setCatalog(catalogData);

                // Poblamos los estados atómicos con la respuesta del backend
                setName(eventData.name || "");
                setDate(eventData.date || "");
                setTime(eventData.time || "");
                setTargetAudience(eventData.target_audience || "General");
                setItinerary(eventData.itinerary || []);
                setStaff(eventData.staff || []);

                // Mapeamos el inventario relacional para el formato del selector reactivo
                if (eventData.inventory) {
                    const mappedInventory = eventData.inventory.map((inv: any) => {
                        const itemInCatalog = catalogData.find((c: any) => c.id === inv.item_id);
                        return {
                            item_id: inv.item_id,
                            quantity_used: inv.quantity,
                            name: itemInCatalog ? itemInCatalog.name : (inv.name || "Insumo Desconocido"),
                            unit: itemInCatalog ? itemInCatalog.unit_of_measure : "uds",
                            category: itemInCatalog ? itemInCatalog.category : "General"
                        };
                    });
            ...
            .catch((err: any) => {
                console.error(err);
                notify.error(err.message || "Fallo crítico al sincronizar la información del evento.");
            })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(data.error || "Fallo al actualizar el evento.");

            notify.success("¡Evento e inventario reconfigurados con éxito!");
            router.push("/events");
        } catch (error: any) {
            console.error(error);
            notify.error(error.message || "Error al actualizar el evento.");
        } finally {
            setLoading(false);
        }
    };
    ...
    if (fetchingEvent) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center min-h-[400px] gap-3">
                ...
                <span className="text-xs font-black uppercase tracking-widest text-slate-400 animate-pulse">Sincronizando producción...</span>
            </div>
        );
    }
        ...
        <div className="max-w-5xl mx-auto px-4 py-8 font-medium text-slate-100 flex flex-col gap-8">
            ...
            <div className="border-b border-slate-800 pb-5">
                <h1 className="text-2xl font-black uppercase tracking-tight text-slate-100 sm:text-3xl">
                    Modificar Evento N° {id}
                </h1>
                <p className="text-xs font-bold text-indigo-400 uppercase tracking-wider mt-1">
                    Panel de reconfiguración logística, repertorio dinámico e insumos asignados
                </p>
            </div>
